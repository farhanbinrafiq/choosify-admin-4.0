...
import { Send, Bell, Info, Target, Trash2, RefreshCw } from 'lucide-react';
...
type NotifTab = 'history' | 'create';

export default function NotificationsPage() {
  const [activeTab, setActiveTab] = useState<NotifTab>('history');
  ...
  const handleResend = (n: SentNotification) => {
    ...
    setScheduleForLater(Boolean(n.scheduledFor));
    setScheduleDateTime('');
    setActiveTab('create');
  };

  const tabClass = (tab: NotifTab) =>
    `px-4 py-3 text-xs font-extrabold tracking-wide cursor-pointer border-b-2 ${
      activeTab === tab ? 'text-[#EF3C23] border-[#EF3C23]' : 'text-gray-500 border-transparent'
    }`;
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex border-b border-gray-200 gap-1">
        <button type="button" className={tabClass('history')} onClick={() => setActiveTab('history')}>
          All Notifications ({sentNotifications.length})
        </button>
      ...
      {activeTab === 'history' ? (
        <div className="space-y-6">
          <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm">
            <div className="flex items-start justify-between gap-4 mb-5 flex-wrap">
              <div>
                <h3 className="text-base font-extrabold text-[#0D1B2A]">Notification Overview</h3>
                <p className="text-xs text-gray-500 font-semibold mt-1">Summary of your notification activity</p>
              </div>
              <button
                type="button"
                onClick={() => setActiveTab('create')}
                className="h-9 px-4 rounded-lg bg-[#EF3C23] text-white text-xs font-extrabold"
              >
                + Create Notification
              </button>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <div className="rounded-xl bg-orange-50 p-4">
                <div className="text-[10px] font-extrabold text-orange-900 tracking-wide">TOTAL SENT</div>
                <div className="text-2xl font-extrabold mt-1">{sentNotifications.length}</div>
              </div>
              <div className="rounded-xl bg-blue-50 p-4">
                <div className="text-[10px] font-extrabold text-blue-800 tracking-wide">TOTAL REACH</div>
                <div className="text-2xl font-extrabold mt-1">128.4K</div>
              </div>
              <div className="rounded-xl bg-green-50 p-4">
                <div className="text-[10px] font-extrabold text-green-800 tracking-wide">OPEN RATE</div>
                <div className="text-2xl font-extrabold mt-1">68.4%</div>
              </div>
              <div className="rounded-xl bg-pink-50 p-4">
                <div className="text-[10px] font-extrabold text-pink-800 tracking-wide">CLICK RATE</div>
                <div className="text-2xl font-extrabold mt-1">24.7%</div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-sm">
            <div className="p-4 border-b border-gray-100">
              <h3 className="text-base font-extrabold text-[#0D1B2A]">All Notifications &amp; History</h3>
              <p className="text-xs text-gray-500 font-semibold mt-1">Sent and scheduled blasts for your workspace</p>
            </div>
            {sentNotifications.length === 0 ? (
              <div className="p-12 text-center">
                <p className="text-sm font-extrabold text-[#0D1B2A]">No notifications yet</p>
                <p className="text-xs text-gray-500 mt-2 mb-4">Create your first blast to start building history.</p>
                <button
                  type="button"
                  onClick={() => setActiveTab('create')}
                  className="h-10 px-5 rounded-lg bg-[#EF3C23] text-white text-xs font-extrabold"
                >
                  Create Notification
                </button>
              </div>
                    ...
                    <div key={n.id} className="p-4 flex items-center justify-between gap-4 hover:bg-gray-50/40 transition-all">
                      <div className="flex items-center gap-4 flex-1 min-w-0">
                        ...
                        <div className="flex-1 min-w-0">
                          <h4 className="text-[13px] font-bold text-[#0D1B2A] truncate" title={n.fullMessage}>
                            {n.title}
                          </h4>
                          <p className="text-[11px] text-gray-400">
                            Targeted to: <span className="text-[#0D1B2A]">{n.target}</span> • Type:{' '}
                            <span className="text-[#0D1B2A]">{n.messageType}</span>
                          </p>
                        </div>
                      </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-8 shadow-sm">
          <div className="flex items-center gap-4 mb-8">
            ...
            <div>
              <h3 className="text-lg font-bold text-[#0D1B2A]">Create New Notification</h3>
          ...
          <div className="space-y-4">
              ...
              <div className="flex justify-between items-center mb-2">
                ...
                <span
                  className={`text-[10px] font-bold ${
                    messageContent.length >= 270
                      ? 'text-red-500'
                      : messageContent.length >= 240
                        ? 'text-amber-500'
                        : 'text-gray-400'
                  }`}
                >
            ...
            <div className="pt-2 border-t border-gray-150/50 mt-2">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <label
                    ...
                    className="text-xs font-bold text-[#0D1B2A] uppercase cursor-pointer select-none"
                  >
                    Schedule for Later
                  </label>
                </div>
              ...
              {scheduleForLater && (
                <div className="mt-3 flex flex-col gap-2 p-4 bg-gray-50 border border-gray-100 rounded-xl">
                  <label className="text-[9px] font-black tracking-widest text-[#0D1B2A] uppercase">
