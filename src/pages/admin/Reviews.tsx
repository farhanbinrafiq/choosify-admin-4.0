    ...
    <div className="space-y-6 text-app-text-primary font-sans animate-in fade-in duration-300">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="space-y-1">
          <span className="text-[9.5px] font-black uppercase tracking-widest text-[#EF3C23] block">Unified Trust & Review Moderation</span>
          ...
          <button
            ...
            className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-extrabold uppercase tracking-wider transition-all shadow-md cursor-pointer${
              isScanning 
                ...
                : 'bg-gradient-to-r from-[#EF3C23] to-[#EF3C23] text-white hover:opacity-90 active:scale-95 shadow-[#EF3C23]/20'
            }`}
          >
      ...
      <AnimatePresence>
        {isScanning && (
          ...
          >
            <div className="bg-app-card border border-app-accent/20 p-5 rounded-2xl space-y-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <Activity className="w-4 h-4 text-[#EF3C23] animate-pulse" />
                ...
                <span className="text-xs font-mono font-bold text-[#EF3C23]">{scanProgress}%</span>
              </div>
              <div className="w-full bg-app-bg h-2.5 rounded-full overflow-hidden border border-app-border">
                <motion.div 
                  className="h-full bg-gradient-to-r from-[#EF3C23] to-amber-500 rounded-full"
                  initial={{ width: '0%' }}
      ...
      {isAdmin && (
        <div className="bg-[#1A1A2E]/30 border border-app-border/70 p-4 rounded-xl">
          <div className="flex items-center justify-between cursor-pointer" onClick={() => setShowShortcutsHUD(!showShortcutsHUD)}>
            <div className="flex items-center gap-2">
              <Keyboard className="w-4 h-4 text-[#EF3C23]" />
            ...
            {showShortcutsHUD && (
              ...
              >
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 pt-2 text-[11px] font-mono border-t border-app-border">
                  <div className="flex items-center gap-2">
                    <kbd className="px-2 py-0.5 bg-app-card border border-app-border rounded text-[#EF3C23]">Tab</kbd>
                    <span className="text-app-text-secondary">Cycle focused review</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <kbd className="px-2 py-0.5 bg-app-card border border-app-border rounded text-[#EF3C23]">Q</kbd>
                    <span className="text-app-text-secondary">Approve & Publish review</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <kbd className="px-2 py-0.5 bg-app-card border border-app-border rounded text-[#EF3C23]">R</kbd>
                    <span className="text-app-text-secondary">Reject & Archive review</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <kbd className="px-2 py-0.5 bg-app-card border border-app-border rounded text-[#EF3C23]">F</kbd>
      ...
      <div className="space-y-4 bg-app-card p-4 rounded-xl border border-app-border">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              ...
              return (
                <button 
                  ...
                  className={`px-3 py-1.5 text-[9.5px] uppercase font-bold tracking-widest cursor-pointer transition-all rounded-[3px] whitespace-nowrap${
                    activeTab === tab 
                      ? 'bg-[#EF3C23]/10 text-[#EF3C23] border border-[#EF3C23]/20' 
          ...
          <div className="relative group w-full md:w-72">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-app-text-secondary group-focus-within:text-[#EF3C23]" />
            <input 
              type="text" 
              placeholder="Search reviewer, SKU, comment..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="pl-9 pr-4 py-2 bg-app-bg border border-app-border rounded-[4px] text-xs w-full focus:outline-none focus:border-[#EF3C23]/40 text-app-text-primary placeholder-slate-500 font-medium"
            />
          </div>
        </div>
              ...
              {(['All', 1, 2, 3, 4, 5] as const).map(rate => (
                <button
                  ...
                  className={`px-2 py-0.5 text-[9px] font-mono font-bold rounded${
                    filterRating === rate ? 'bg-[#EF3C23]/20 text-[#EF3C23]' : 'text-slate-400 hover:text-white'
                  }`}
                >
              ...
              {(['All', 'Authentic Only', 'Flagged/Spam Only'] as const).map(auth => (
                <button
                  ...
                  className={`px-2 py-0.5 text-[9px] font-bold rounded${
                    filterAuthenticity === auth ? 'bg-[#EF3C23]/20 text-[#EF3C23]' : 'text-slate-400 hover:text-white'
                  }`}
                >
                  {auth}
                </button>
              ...
              return (
                ...
                >
                    ...
                    <div className="flex gap-3 items-center">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-app-text-secondary mt-0.5">
                          SKU: <span className="text-app-text-secondary font-semibold">{rev.product}</span> · Merchant: <span className="font-bold text-[#EF3C23]">{rev.store}</span>
                        </p>
                      </div>
                    </div>
                  ...
                  {editingReviewId === rev.id ? (
                    <div className="space-y-2 mb-4 bg-app-bg p-3 rounded-lg border border-app-border" onClick={(e) => e.stopPropagation()}>
                      ...
                      <textarea 
                        ...
                        className="w-full bg-app-card border border-app-border rounded p-2 text-xs text-app-text-primary focus:outline-none focus:border-[#EF3C23]"
                        rows={2}
                      />
                  ...
                  <div className="space-y-3 pt-3 border-t border-app-border" onClick={(e) => e.stopPropagation()}>
                    {rev.response ? (
                      <div className="flex gap-2 items-start text-xs border-l-2 border-[#EF3C23] pl-3.5 py-1">
                        <CornerDownRight className="w-3.5 h-3.5 text-[#EF3C23] shrink-0 mt-0.5" />
                        <div className="flex-1">
                          <div className="flex items-center gap-2 flex-wrap">
                            <span className="font-extrabold text-app-text-primary text-[11px]">{rev.response.author}</span>
                            <span className="bg-[#EF3C23]/15 text-[#EF3C23] px-1.5 py-0.2 text-[7px] font-black rounded uppercase tracking-wider">Official Response</span>
                      ...
                      (isSeller || isAdmin) && (
                        <div className="space-y-2">
                          <div className="relative flex items-center bg-app-bg p-1.5 rounded-lg border border-app-border focus-within:border-[#EF3C23]/30">
                            <input 
                              type="text"
                            ...
                            <button 
                              ...
                              className="p-1.5 text-[#EF3C23] hover:text-[#EF3C23]-light cursor-pointer"
        ...
        <div className="space-y-6">
          ...
          <div className="bg-app-card p-5 rounded-xl border border-app-border space-y-4">
            <h3 className="text-xs font-black text-app-text-primary uppercase tracking-wider flex items-center gap-2">
              <Sliders className="w-4 h-4 text-[#EF3C23]" />
                ...
                <div className="space-y-4">
                  <div className="p-3 bg-app-bg rounded-lg space-y-2 border border-app-border">
                    ...
                    <div className="text-[10px] text-app-text-secondary leading-relaxed font-mono mt-1">
                      Posted via Web Ingress Node · Verified Client Purchase ID: <span className="text-[#EF3C23]">ORD-9283-REK</span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <textarea
                      ...
                      className="w-full bg-app-bg border border-app-border rounded-lg p-3 text-xs text-app-text-primary placeholder-slate-600 focus:outline-none focus:border-[#EF3C23] min-h-[90px] font-medium"
                    />
                    <div className="flex justify-end">
                      <span className="text-[9px] text-[#EF3C23] font-mono font-bold uppercase tracking-wider">Auto-Saved to Local Node Logs</span>
                    </div>
                  </div>
          ...
          <div className="bg-app-card p-5 rounded-xl border border-app-border space-y-4">
            <h3 className="text-xs font-black text-app-text-primary uppercase tracking-wider flex items-center gap-2">
              <ShieldAlert className="w-4 h-4 text-[#EF3C23]" />
              Policy Summary
            </h3>
            <ul className="space-y-2.5 text-xs text-app-text-secondary leading-relaxed">
              <li className="flex items-start gap-2">
                <div className="w-1.5 h-1.5 rounded-full bg-[#EF3C23] shrink-0 mt-1.5" />
