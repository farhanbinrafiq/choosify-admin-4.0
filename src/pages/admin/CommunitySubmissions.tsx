  ...
  return (
    <div className="space-y-6 text-[#1A1A2E] bg-gray-50/50 min-h-screen p-1 text-left">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <span className="text-[9.5px] font-black uppercase tracking-widest text-[#EF3C23] block mb-1">Consumer Moderation Logs</span>
        ...
        {[
          ...
          <div key={s.label} className="bg-white p-5 rounded-xl border border-gray-150 border-l-[4px] shadow-sm flex flex-col justify-between">
             ...
             <div className="flex items-center justify-between border-t border-gray-50 pt-2 text-[10px]">
                <span className="text-app-text-secondary">{s.subtitle}</span>
      ...
      <div className="bg-white p-4 rounded-xl border border-gray-150 shadow-sm flex flex-col lg:flex-row gap-4 items-center justify-between">
        ...
        {/* Searching text */}
        <div className="relative w-full lg:w-72 bg-[#F8F9FA] border border-gray-200 rounded-lg flex items-center px-3 py-1.5 gap-2 focus-within:border-[#EF3C23] transition-colors">
        ...
        <div className="flex flex-col gap-2">
          <div className="bg-app-card p-4 rounded-xl shadow-lg flex items-center justify-between text-app-text-primary animate-fade-in">
            <div className="flex items-center gap-2.5 text-xs">
              <CheckSquare className="w-4 h-4 text-[#EF3C23]" />
              <span className="font-bold">
                Bulk Actions: <strong className="text-[#EF3C23] font-extrabold">{selectedIds.length}</strong> items selected
              </span>
            </div>
            <div className="flex items-center gap-2">
              <button
              ...
              <button
                ...
                className="px-2.5 py-1.5 text-[10px] uppercase font-bold text-app-text-secondary hover:text-white"
              >
                Cancel
              </button>
            </div>
          ...
          {showBulkRejectForm && (
            <div className="bg-[#1F1F35] p-4 rounded-xl border border-rose-500/30 text-app-text-primary flex flex-col gap-3 shadow-2xl">
              <div className="flex flex-col gap-1">
                <span className="text-[10px] font-black text-rose-400 uppercase tracking-wider">Specify Bulk Rejection Reason</span>
                <span className="text-[9px] text-app-text-secondary">All selected {selectedIds.length} submissions will be rejected with this description:</span>
              </div>
              <textarea
                value={bulkRejectReason}
                onChange={e => setBulkRejectReason(e.target.value)}
                rows={2}
                className="w-full px-3 py-2 bg-app-card/20 border border-app-border rounded-lg text-xs font-semibold outline-none focus:border-rose-500 text-app-text-secondary"
              ...
              <div className="flex gap-2 justify-end">
                ...
                <button
                  ...
                  className="px-3 py-1.5 bg-white/5 hover:bg-white/10 text-app-text-secondary text-[9px] font-black uppercase tracking-widest rounded-lg cursor-pointer transition-colors"
                >Cancel</button>
              </div>
            </div>
          )}
      ...
      <div className="space-y-4">
        ...
        {filteredSubmissions.length > 0 && (
          <div className="flex items-center px-4 py-2 bg-white/40 rounded-lg text-[10px] text-app-text-secondary font-bold uppercase tracking-wider justify-between border border-gray-100">
             <button 
              ...
              <div className="p-5 flex flex-col md:flex-row items-start gap-4 md:items-center justify-between">
                ...
                <div className="flex items-start gap-4 flex-1 min-w-0">
                  ...
                  <div className="relative shrink-0">
                    ...
                    />
                    <span className="absolute bottom-1 right-1 bg-app-bg/10 backdrop-blur text-app-text-primary text-[8px] px-1.5 py-0.5 rounded font-mono uppercase tracking-tighter">
                      {sub.productType}
                    </span>
                  </div>
                  ...
                  <div className="space-y-1 text-left flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-1.5">
                      <span className="text-[10px] font-bold text-app-text-secondary uppercase tracking-wider">{sub.brand}</span>
                    ...
                    <div className="flex items-center gap-2">
                       ...
                       <span className="text-[9px] bg-amber-50 text-[#EF3C23] font-black px-1.5 py-0.5 rounded border border-amber-100 uppercase tracking-tighter">
                         {sub.discount}% OFF
                       </span>
                    </div>
                    ...
                    {sub.socialProof && (
                      <div className="text-[10px] text-app-text-secondary italic flex items-center gap-1">
                        <Sparkles className="w-3 h-3 text-[#EF3C23] shrink-0" />
                        <span>* "{sub.socialProof}"</span>
                      </div>
                    )}

                    {/* Creator / Submitter footprint */}
                    <div className="text-[9.5px] text-app-text-secondary font-bold uppercase tracking-widest pt-1">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="space-y-1">
                        <span className="text-[9px] font-black text-app-text-secondary uppercase tracking-widest block">Full Description Context</span>
                      ...
                      <div className="grid grid-cols-2 gap-3 text-[11px]">
                         <div className="bg-white p-2 rounded-lg border border-gray-150">
                            ...
                            <span className="font-mono font-bold text-[#EF3C23]">
                              {sub.promoCode || 'NO PROMO APPLIES'}
                            </span>
                         </div>
                         <div className="bg-white p-2 rounded-lg border border-gray-150">
                            <span className="text-[8.5px] font-bold text-app-text-secondary uppercase tracking-widest block">Valid Until</span>
                    ...
                    <div className="space-y-3 flex flex-col justify-between">
                      ...
                      <div className="space-y-1 flex-1">
                        <span className="text-[9px] font-black text-app-text-secondary uppercase tracking-widest block">Internal Admin Notes</span>
                        <textarea 
                          ...
                          rows={3}
                          className="w-full bg-white border border-gray-200 rounded-lg p-3 text-xs outline-none focus:border-[#EF3C23] text-[#1A1A2E] font-medium resize-none"
                        />
                      </div>
                      ...
                      <div className="flex justify-end gap-2 text-right pt-2 border-t border-gray-100">
                        <button
                          ...
                          className="px-3 py-1.5 bg-app-card hover:bg-indigo-950 text-app-text-primary text-[10px] uppercase font-bold tracking-wider rounded-md border-none cursor-pointer transition-colors"
                        >
                          Save Admin Note
                        </button>
                      </div>
        ...
        {filteredSubmissions.length === 0 && (
          <div className="bg-white rounded-xl border border-dashed border-gray-300 p-12 text-center text-xs text-app-text-secondary italic">
