      ...
      {enabled.map((blk) => (
        <div key={blk.id} className="bg-[#F4F7F9] rounded-[10px] px-5 py-[18px] flex flex-col gap-3 text-left">
          <div className="flex items-center gap-2 text-[12px] font-extrabold text-[#1A1A2E]">
            <span className="text-[#EF3C23]">{quadrantIcon(blk.title)}</span>
            {blk.title}
          </div>
          {blk.content ? (
              ...
              {blk.bullets.map((item, i) =>
                blk.listStyle === 'numbered' ? (
                  <div key={i} className="flex items-start gap-2">
                    <span className="text-[#EF3C23] font-black shrink-0">{i + 1}.</span>
                    <span>{item}</span>
                  </div>
                ) : (
    ...
    <div className="bg-[#F4F7F9] rounded-[10px] p-4 space-y-3 text-left border border-[#E8EDF2]">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 flex-grow min-w-0">
          <span className="text-[#EF3C23] shrink-0">{quadrantIcon(block.title)}</span>
          <input
            value={block.title}
        ...
        <>
          <div className="space-y-2">
            ...
            <button
              ...
              className="inline-flex items-center gap-1 text-[9px] font-black uppercase tracking-wider text-[#EF3C23] bg-transparent border-0 cursor-pointer"
            >
