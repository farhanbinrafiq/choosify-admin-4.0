/**
 * Messaging Firestore collection names — IS-005 / ES-001.
 * Prefixed consistently with other commerce domains (commerce_*, omni_*).
 */

export const MESSAGING_CONVERSATIONS = 'messaging_conversations';
export const MESSAGING_MESSAGES = 'messaging_messages';
export const MESSAGING_ATTACHMENTS = 'messaging_attachments';
export const MESSAGING_SOCIAL_INBOX = 'messaging_social_inbox';
export const MESSAGING_SUPPORT_TICKETS = 'messaging_support_tickets';
export const MESSAGING_ADMIN_ENTRIES = 'messaging_admin_entries';
