...
export function inquiryDetailRows(lead: OpsLead): Array<[string, string]> {
  const rows: Array<[string, string | undefined]> = [
    ['Reference', lead.referenceId],
    ['Type', inquiryTypeLabel(lead.inquiryType)],
    [lead.inquiryType === 'general_contact' ? 'Subject' : 'Brand / company', lead.brandName],
    ['Contact', lead.contactPerson],
    ['Email', lead.email],
    ['Website / social', lead.website],
    ['Category', lead.categoryName],
    ['Country', lead.country],
    ['Partnership model', PARTNERSHIP_MODELS.find((m) => m.value === lead.partnershipModel)?.label],
    ['Budget', AD_BUDGET_RANGES.find((b) => b.value === lead.budget)?.label],
    ['Placement interest', buildAdPlacementInterests().find((p) => p.value === lead.placementInterest)?.label],
  ];
...
export async function notifyInquiryCreated(lead: OpsLead): Promise<OpsLeadDelivery> {
  ...
  const rows = inquiryDetailRows(lead);
  const html = `<div style="font-family:Arial,sans-serif;font-size:14px;color:#172033">
<h2 style="margin:0 0 12px">${esc(title)}</h2>
<table cellpadding="6" style="border-collapse:collapse">${rows
    .map(([k, val]) => `<tr><td style="color:#667085;vertical-align:top">${esc(k)}</td><td>${esc(val)}</td></tr>`)
    .join('')}</table>
${lead.message ? `<p style="white-space:pre-wrap;border-left:3px solid #E5E7EB;padding-left:10px">${esc(lead.message)}</p>` : ''}
${link ? `<p><a href="${esc(link)}">Open in the Choosify dashboard</a></p>` : ''}
</div>`;
  ...
  for (const to of recipients) {
    try {
      const result = await sendEmail({ to, subject: `${title}${lead.referenceId ? ` · ${lead.referenceId}` : ''}`, html, text });
