...
async function main() {
  ...
  const sellerAReg = await registerConsumer(`escrow-sa-${RUN_ID}@probe.local`);
  const sellerA = await upgradeToSeller(sellerAReg.token, `EscrowStoreA ${RUN_ID}`);
  const sellerBReg = await registerConsumer(`escrow-sb-${RUN_ID}@probe.local`);
  const sellerB = await upgradeToSeller(sellerBReg.token, `EscrowStoreB ${RUN_ID}`);
