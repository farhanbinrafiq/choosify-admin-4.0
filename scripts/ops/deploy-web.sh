...
# ============================================================
# deploy-web.sh -- manual, operator-invoked deployment for
# choosify-web (Hostinger VPS).
#
# This script is NEVER triggered automatically (no CI, no git
# hook). It never touches .env, Nginx, TLS, UFW, SSH,
# PostgreSQL, cron, or Vercel.
#
# DEPLOYMENT STATE MARKER
# ------------------------------------------------------------
# /var/www/choosify/deploy/state/web.deployed-commit records the
# commit that is ACTUALLY currently being served, which can
...
#
# Usage:
#   ./deploy-web.sh              real deployment
#   ./deploy-web.sh --dry-run    inspection only, no changes
# ============================================================

REPO_DIR="/var/www/choosify/web"
APP_NAME="choosify-web"
APP_PORT="3000"
ARTIFACT_DIR="dist-server"
ARTIFACT_ENTRY="dist-server/server.cjs"
LOCAL_HEALTH_URL="http://127.0.0.1:3000"
PUBLIC_URL_1="https://choosify.bd"
PUBLIC_URL_2="https://www.choosify.bd"
DEPLOY_ROOT="/var/www/choosify/deploy"
LOCK_FILE="$DEPLOY_ROOT/locks/deploy-web.lock"
MARKER_FILE="$DEPLOY_ROOT/state/web.deployed-commit"
HEALTH_ATTEMPTS=12
HEALTH_INTERVAL=2
...
# See deploy-admin.sh for the full rationale: this only ever
# acts when the artifact directory is actually missing, which
# is only possible in the narrow window between the pre-build
# move-aside and a completed build (success or the explicit
# failure-handler both leave the directory present again).
...
run_all_health_checks() {
  ...
  wait_for_http "$LOCAL_HEALTH_URL" "$HEALTH_ATTEMPTS" "$HEALTH_INTERVAL" 0 || return 1
  ...
  wait_for_http "$PUBLIC_URL_2" "$HEALTH_ATTEMPTS" "$HEALTH_INTERVAL" 0 || return 1
...
exec 200>"$LOCK_FILE"
if ! flock -n 200; then
  log ERROR "Another deploy-web.sh run is already in progress. Aborting."
  exit 1
fi

log INFO "=== deploy-web.sh started$( [ "$DRY_RUN" -eq 1 ] && echo ' (DRY RUN)' ) ==="
...
# ------------------------------------------------------------
# See deploy-admin.sh for the full rationale. One-time
# "bootstrap current known-good production commit" action.
# ============================================================
if [ ! -f "$MARKER_FILE" ]; then
  ...
  LOCAL_CODE=$(http_code "$LOCAL_HEALTH_URL")
  if [ "$LOCAL_CODE" != "200" ]; then
    log ERROR "[BOOTSTRAP] local health did not return 200 (got $LOCAL_CODE)."
    BOOTSTRAP_OK=0
  fi
  PUB1_CODE=$(http_code "$PUBLIC_URL_1")
  if [ "$PUB1_CODE" != "200" ]; then
    log ERROR "[BOOTSTRAP] public apex did not return 200 (got $PUB1_CODE)."
    BOOTSTRAP_OK=0
  fi
  PUB2_CODE=$(http_code "$PUBLIC_URL_2")
  if [ "$PUB2_CODE" != "200" ]; then
    log ERROR "[BOOTSTRAP] public www did not return 200 (got $PUB2_CODE)."
    BOOTSTRAP_OK=0
  fi
...
# ============================================================
# INCOMING CHANGE INSPECTION -- diffed from the marker
# ============================================================
