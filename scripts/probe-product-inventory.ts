 ...
 * This mirrors the pattern already proven in probe-brand-ownership.ts.
 */
async function partnerApplySeller(email: string, password: string, storeName: string) {
...
async function approvePendingByEmail(adminToken: string, email: string) {
  ...
  const approveRes = await fetch(`${base}/operations/partner-applications/${app.id}/approve`, {
    ...
    body: JSON.stringify({ note: 'product inventory probe' }),
  });
  return { status: approveRes.status, body: await json(approveRes) };
}

/**
 * apply -> admin-approve -> login -> grant Marketplace Access on the seller's
 * auto-provisioned brand, returning the same {token, uid} shape upgradeToSeller used
 * to. The legacy endpoint self-granted Marketplace Access instantly; the real
 * lifecycle requires this as an explicit Admin action, so the fixture does it here.
 */
async function upgradeToSeller(adminToken: string, email: string, password: string, storeName: string) {
  const apply = await partnerApplySeller(email, password, storeName);
  if (apply.status !== 201) return { status: apply.status, body: apply.body as { accessToken?: string; uid?: string } };
  const approve = await approvePendingByEmail(adminToken, email);
  if (approve.status !== 200) return { status: approve.status, body: {} as { accessToken?: string; uid?: string } };
  const loggedIn = await login(email, password);
  const ownBrands = await fetch(`${base}/catalog/brands`, { headers: { Authorization: `Bearer ${loggedIn.token}` } });
  ...
  return { status: 200, body: { accessToken: loggedIn.token, uid: loggedIn.uid } };
}
...
async function main() {
  ...
  const sellerAEmail = `probe.prod.a.${RUN_ID}@choosify.test`;
  const sellerAPassword = 'Probe!2026xx';
  const upgradeA = await upgradeToSeller(admin.token, sellerAEmail, sellerAPassword, `Probe Store A ${RUN_ID}`);
  assert(upgradeA.status === 200, 'Seller A provisioned via Partner Application + Admin approval', upgradeA.status);
  const sellerAToken = upgradeA.body.accessToken as string;
  const sellerAUid = upgradeA.body.uid as string;

  const sellerBEmail = `probe.prod.b.${RUN_ID}@choosify.test`;
  const sellerBPassword = 'Probe!2026xx';
  const upgradeB = await upgradeToSeller(admin.token, sellerBEmail, sellerBPassword, `Probe Store B ${RUN_ID}`);
